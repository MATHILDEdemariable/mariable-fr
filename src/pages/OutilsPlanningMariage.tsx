        ...
        <section className="pb-12 px-4" style={{ paddingTop: 'var(--header-h)' }}>
          <div className="container mx-auto max-w-6xl">
            <div className="text-center mb-12">
              ...
              <h1 className="text-3xl md:text-4xl lg:text-5xl font-serif mb-6 text-editorial-noir">
                Outils planning mariage
              </h1>
              <p className="text-lg md:text-xl text-editorial-noir/60 max-w-3xl mx-auto mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
              {processSteps.map((step) => (
                <div key={step.step} className="text-center">
                  ...
                  <p className="text-sm text-editorial-noir/60">{step.description}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="container mx-auto max-w-6xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {tools.map((tool, index) => (
                <Card 
                  key={index}
                  className="cursor-pointer transition-all duration-300 border-editorial-noir/10 bg-white rounded-none hover:shadow-lg hover:scale-105"
                  onClick={() => navigate(tool.path)}
                >
                  <CardHeader>
                    <div className="flex items-center gap-3 mb-2">
                      <div className="text-editorial-noir">
                        {tool.icon}
                      </div>
                      <CardTitle className="font-serif text-xl text-editorial-noir">
                        {tool.title}
                      </CardTitle>
                    </div>
                  </CardHeader>
                  <CardContent>
                    <p className="text-editorial-noir/60 mb-4">{tool.description}</p>
                    <ul className="space-y-1 mb-4">
                      {tool.features.map((feature, featureIndex) => (
                        <li key={featureIndex} className="flex items-center text-sm text-editorial-noir/70">
