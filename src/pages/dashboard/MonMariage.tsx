...
const MonMariage = () => {
  ...
  return (
    <div className="space-y-8">
              ...
              <Card key={retro.id} className="hover:shadow-lg transition-shadow">
                <CardHeader>
                  <CardTitle className="text-lg flex items-center justify-between">
                    <span className="truncate">Rétroplanning {retro.wedding_date ? format(new Date(retro.wedding_date), 'dd MMMM yyyy', { locale: fr }) : ''}</span>
                  </CardTitle>
                  <CardDescription>
                    Créé le {format(new Date(retro.created_at), 'dd MMM yyyy', { locale: fr })}
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-2 text-sm text-muted-foreground">
                    {retro.wedding_date && (
                      <div className="flex items-center gap-2">
                        ...
                        <span>{format(new Date(retro.wedding_date), 'dd MMMM yyyy', { locale: fr })}</span>
                      </div>
                    )}
                  </div>
                ...
                <CardContent>
                  <div className="flex gap-2">
                    <Button
                      ...
                      size="sm"
                      className="flex-1"
                      onClick={() => navigate(`/vibe-wedding?session=${convo.session_id}`)}
                    >
                      <Eye className="h-4 w-4 mr-1" />
                      Reprendre
                    </Button>
      ...
      {/* État vide */}
      {projects.length === 0 && vibeConversations.length === 0 && retroplannings.length === 0 && (
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                ...
                <Button variant="outline" onClick={() => navigate('/vibe-wedding')}>
                  <Plus className="h-4 w-4 mr-2" />
                  Nouveau projet IA
                </Button>
              </div>
