...
const ProjectSummary = () => {
  ...
  return <div className="space-y-8">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-wedding-olive/20 p-4 space-y-3">
        {/* Guide de démarrage */}
        <button onClick={() => setShowGuideModal(true)} className="flex items-center gap-3 w-full text-left hover:bg-gray-50 transition-colors rounded-lg p-2 -m-2">
        ...
        {/* Vidéo tutorielle */}
        <button onClick={() => setShowVideoModal(true)} className="flex items-center gap-3 w-full text-left hover:bg-gray-50 transition-colors rounded-lg p-2 -m-2">
        ...
        <div className="mt-3 bg-gradient-to-r from-purple-50 via-pink-50 to-purple-50 border border-purple-200 rounded-lg p-4 flex items-center justify-between gap-4">
          ...
          <Button variant="outline" size="sm" onClick={() => window.open('https://www.instagram.com/mariable.fr/', '_blank', 'noopener,noreferrer')} className="shrink-0 border-purple-300 hover:bg-purple-50">
            Suivre
          </Button>
        </div>
      </div>
      ...
      <Dialog open={showVideoModal} onOpenChange={setShowVideoModal}>
        <DialogContent className="sm:max-w-3xl">
          ...
          <LoomVideoEmbed videoId={TUTORIAL_VIDEOS.welcome.loomId} title={TUTORIAL_VIDEOS.welcome.title} description={TUTORIAL_VIDEOS.welcome.description} />
        </DialogContent>
      </Dialog>
